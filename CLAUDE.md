...
- **Bottom sheet:** All controls live in a fixed bottom sheet. Collapsed state shows a 44px peek bar with a drag handle nub and settings summary string (e.g. `C Maj · E Shape · Triads`). Expanded state reveals a two-panel layout: left panel has a 12-row × 2-column grid of major/minor key buttons (reusing desktop `STYLE.keyBtn`/`STYLE.minorKeyBtn`); right panel stacks shapes (2-column grid with "All" spanning full width), Intervals/Notes toggle, and options (Triads, Penta, Quality — one per row). Controlled by `sheetOpen` state. Supports tap toggle and touch drag gestures (swipe up to open, swipe down to close, 40px threshold). A backdrop overlay closes the sheet on tap. The sheet uses `position: fixed` with `transform: translateY()` animation.
